import { Booking, timeToMinutes } from "@/features/time-map/lib/booking-data";
        ...
        return (
          <button
            ...
            className={`relative w-full rounded-xl border px-4 py-3 text-left shadow-sm transition-all duration-300 active:scale-[0.98] ${
              isActive
                ? "border-booking-past/70 bg-booking-past/20"
                : isPast
                ? "border-booking-past/60 bg-booking-past/20"
                : booking.status === "conflict"
                ? "border-booking-conflict/60 bg-booking-conflict/15"
                : booking.status === "confirmed"
                ? "border-booking-confirmed/60 bg-booking-confirmed/15"
                : "border-booking-pending/60 bg-booking-pending/15"
            }`}
          >
            {/* Active indicator */}
            {isActive && (
